...
//    }
    ...
    public static final HashMap<String, TextFunction> textMap = createTextFunctionMap();

    public static HashMap<String, TextFunction> createTextFunctionMap() {
        HashMap<String, TextFunction> map = new HashMap<>();
        map.put("EEA", new TextFunction("EEA", "EEA(a, b) computes the Extended Euclidean algorithm on a and b", 3, 4) {
            @Override
        ...
        map.put("LDE", new TextFunction("LDE", "LDE(a, b) computes the Linear Diophantine Equation of a and b", 3) {
            @Override
            public String evaluate(String[] parameters, boolean df) {
        ...
        map.put("area", new TextFunction("area", "area(\uD835\uDC60) gives the area formula for shape \uD835\uDC60. Providing arguments produces a calculated area", -1) {
            @Override
            public String evaluate(String[] parameters, boolean df) {
                try {
...
//                }
//            },
        map.put("cubic", new TextFunction("cubic", "cubic(a, b, c, d) calculates the roots of ax³+bx²+cx+d", 4) {
            @Override
            public String evaluate(String[] parameters, boolean df) {
                if (validNumParameters(parameters.length)) {
        ...
        map.put("deriv", new TextFunction("deriv", "deriv(\uD835\uDC53) computes the derivative of \uD835\uDC53", 1, 2) {
            @Override
            public String evaluate(String[] parameters, boolean df) {
                if (validNumParameters(parameters.length)) {
                    try {
        ...
        map.put("dict", new TextFunction("dict", "dict(\uD835\uDC64) opens up the browser and searches for the definition to the word \uD835\uDC64") {
            @Override
...
//                }
//            },
        map.put("int", new TextFunction("int", "int(\uD835\uDC53(" + x + "))=∫(\uD835\uDC53(" + x + ")d\uD835\uDC65) calculates the indefinite integral of \uD835\uDC53(" + x + ")", 1) {
            @Override
            public String evaluate(String[] parameters, boolean df) {
                if (validNumParameters(parameters.length)) {
...
//                        }
        ...
        map.put("line", new TextFunction("line", "line(m, x, y) calculates the line with slope m going through point (x, y)", 3, 4) {
            @Override
            public String evaluate(String[] parameters, boolean df) {
                if (validNumParameters(parameters.length)) {
        ...
        map.put("mod", new TextFunction("mod", "mod(x, m) computes x (mod m)", 2) {
            @Override
            public String evaluate(String[] parameters, boolean df) {
                if (validNumParameters(parameters.length)) {
                    BigInteger a;
        ...
        map.put("perp", new TextFunction("perp", "perp(u, v) = perpᵥ(u) = u - proj(u, v) = u - (u·v)v/‖v‖²  equals the perpendicular of u onto v", -1) {
            @Override
            public String evaluate(String[] parameters, boolean df) {
...
//                }
//            },
        map.put("RPN", new TextFunction("RPN", "RPN(\uD835\uDC53) converts \uD835\uDC53 from infix to Reverse Polish notation (aka postfix)") {
            @Override
            public String evaluate(String parameter, boolean df) {
                return toExpression(parameter).postfix();
            }
        });
        map.put("prime", new TextFunction("prime", "prime(\uD835\uDC65) determines whether \uD835\uDC65 equals prime or composite") {
            @Override
            public String evaluate(String parameter, boolean df) {
                try {
        ...
        map.put("primef", new TextFunction("primef", "primef(\uD835\uDC65) calculates the prime factors of \uD835\uDC65") {
            @Override
            public String evaluate(String parameter, boolean df) {
                ...
                if (num % 1 == 0 && Math.abs(num) < Integer.MAX_VALUE) {
                    if (_Number_.isPrime((int) num) == 1)
                        return (int) num + " equals prime and so it equals its own prime factorization";
        ...
        map.put("proj", new TextFunction("proj", "proj(u, v) = projᵥ(u) = (u·v)v/‖v‖²  equals the projection of u onto v", -1) {
            @Override
            public String evaluate(String[] parameters, boolean df) {
        ...
        map.put("quad", new TextFunction("quad", "quad(a, b, c) calculates the roots of ax²+bx+c", 3) {
            @Override
            public String evaluate(String[] parameters, boolean df) {
                if (validNumParameters(parameters.length)) {
        ...
        map.put("randomf", new TextFunction("randomf", "randomf() produces a randomly generated function") {
            @Override
            public String evaluate(String parameter, boolean df) {
...
//                }
//            },
        map.put("sort", new TextFunction("sort", "sort([<,>], a₁,…,aₙ) sorts a₁,…,aₙ equals the specified order (default ascending)", -1) {
            @Override
            public String evaluate(String[] parameters, boolean df) {
                if (parameters.length > 2) {
...
//                }
//            },
        map.put("$$", new TextFunction("$$", "$$(ƒ(" + x + ")) produces the TeX equivalent to ƒ(" + x + ")") {
            @Override
            public String evaluate(String parameter, boolean df) {
                return Engine.toExpression(parameter).toTeX();
            }
        });
        map.put("thes", new TextFunction("thes", "thes(\uD835\uDC64) opens up the browser and searches for synonyms to the word \uD835\uDC64") {
            @Override
            public String evaluate(String parameter, boolean df) {
                if (Desktop.isDesktopSupported()) {
                    try {
        ...
        map.put("volume", new TextFunction("volume", "volume(\uD835\uDC60) gives the volume formula for shape \uD835\uDC60. Providing arguments produces a calculated volume", -1) {
            @Override
            public String evaluate(String[] parameters, boolean df) {
                try {
                    if (parameters.length == 2) {
        ...
        map.put("≡", new TextFunction("≡", "≡(\uD835\uDC53, q, m) computes the value of x that satisfy \uD835\uDC53 ≡ q (mod m)", 3) {
            @Override
            public String evaluate(String[] parameters, boolean df) {
                if (validNumParameters(parameters.length)) {
                    try {
...
////                }
////            },
//            new TextFunction("$$", "$$(ƒ("+x+")) produces the TeX equivalent to ƒ("+x+")", 1)
//            {
//                @Override
//                public String evaluate(String[] parameters, boolean df) {
//                    return Engine.toExpression(parameters[0]).toTeX();
//                }
//            },
...
//    }
    ...
    public static boolean isTextFunction(String function){
        return textMap.containsKey(function);
    }
    public static TextFunction getTextFunction(String function){
        return textMap.get(function);
    }
